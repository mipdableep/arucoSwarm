...
void aruco_utils::trackMarkerThread() {
    ...
    cv::Mat objPointstest(4, 1, CV_32FC3);
    objPoints = objPointstest;
    objPoints.ptr<cv::Vec3f>(0)[0] = cv::Vec3f(-currentMarkerSize/2.f, currentMarkerSize/2.f, 0);
    objPoints.ptr<cv::Vec3f>(0)[1] = cv::Vec3f(currentMarkerSize/2.f, currentMarkerSize/2.f, 0);
    objPoints.ptr<cv::Vec3f>(0)[2] = cv::Vec3f(currentMarkerSize/2.f, -currentMarkerSize/2.f, 0);
    ...
    while (!stop) {
        ...
        // if the set marker was detected
        if (correct_index != -9) {
            if (ids.empty())
                arucoDetected = false;
            else
            ...
            cv::aruco::estimatePoseSingleMarkers(
                corners, currentMarkerSize, cameraParams[0], cameraParams[1],
                localRvecs, localTvecs);
            
            for (int i = 0; i < ids.size(); i++) {
                cv::drawFrameAxes(imageCopy, cameraParams[0], cameraParams[1],
                                  localRvecs[i], localTvecs[i], 0.1);
            }
            ...
            if (init) {
                ...
                if (!localRvecs.empty()) {
                    ...
                    try {
                        cv::Rodrigues(localRvecs[correct_index], rmat);
                    } catch (...) {
                        std::cout << "coudlnot convert vector to mat"
                                  << std::endl;
                        continue;
                    }
                    auto t = cv::Mat(-rmat.t() * cv::Mat(localTvecs[correct_index]));
                    rightLeft = t.at<double>(0) * 100;
                    upDown = t.at<double>(1) * 100;
                    forward = t.at<double>(2) * 100;
                    ID = ids[correct_index];
                    yaw = getLeftOverAngleFromRotationVector(localRvecs[correct_index]);
                    rollAngle = getHorizontalAngleFromRotationVector(localRvecs[correct_index]);
...
void aruco_utils::getEulerAngles(cv::Mat &rotCameraMatrix, cv::Vec3d &eulerAngles) {
    ...
    double projMatrix[12] = {_r[0], _r[1], _r[2], 0,     _r[3], _r[4],
                             _r[5], 0,     _r[6], _r[7], _r[8], 0};
    decomposeProjectionMatrix(cv::Mat(3, 4, CV_64FC1, projMatrix), cameraMatrix,
                              rotMatrix, transVect, rotMatrixX, rotMatrixY,
